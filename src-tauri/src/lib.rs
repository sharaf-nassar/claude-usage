...
async fn trigger_analysis(app: tauri::AppHandle) -> Result<(), String> {
    let storage = get_storage()?;
    let result = learning::spawn_analysis(storage, "on-demand").await;
...
pub fn run() {
    match Storage::init() {
        ...
        Err(e) => {
            eprintln!("Fatal: failed to initialize storage: {e}");
            std::process::exit(1);
        }
    }
            ...
            if let Some(storage) = STORAGE.get() {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    loop {
                        ...
                        {
                            eprintln!("Periodic usage cleanup error: {e}");
                        }
                        if let Err(e) =
                            tokio::task::block_in_place(|| storage.aggregate_and_cleanup_tokens())
                        {
                            eprintln!("Periodic token cleanup error: {e}");
                        }
                        if let Err(e) =
                            tokio::task::block_in_place(|| storage.cleanup_old_observations())
                        {
                            eprintln!("Periodic observation cleanup error: {e}");
                        }
                    }
                });
                ...
                // Listen for session-end events to trigger analysis (only if enabled)
                {
                    ...
                    app.listen("learning-session-end", move |_event| {
                        ...
                        tauri::async_runtime::spawn(async move {
                            if let Err(e) = learning::spawn_analysis(storage, "session-end").await {
                                eprintln!("Session-end learning analysis error: {e}");
                            }
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    loop {
                        ...
                        if last_run.elapsed() >= std::time::Duration::from_secs(interval_mins * 60)
                        {
                            ...
                            if let Err(e) = learning::spawn_analysis(storage, "periodic").await {
                                eprintln!("Periodic learning analysis error: {e}");
                            }
                        }
                    }
                });
